...
public:
    Paginator(Iterator range_begin, Iterator range_end, size_t page_size) {
        auto it = range_begin;
        while (it != range_end) {
            const size_t current_page_size = min(page_size, static_cast<size_t>(distance(it, range_end)));
            const Iterator page_end = next(it, current_page_size);
...
template<typename Iterator>
ostream& operator<<(ostream& os, const IteratorRange<Iterator>& it_range) {
